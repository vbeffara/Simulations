...
int main (int argc, char ** argv) {
    ...
    int nn=vb::H['n'], nn2=nn/2, mm=vb::H['m'], tt=vb::H['t']; state::a=vb::H['a']; state::b=vb::H['b'];
    std::vector<double> profile(nn,0), boltzmann(mm,0), var(tt,0);

    vb::ProgressBar PB (mm);
    for (int i=0; i<mm; ++i) {
        Model M(nn); M[nn2].v = 1;
        for (int t=0; t<tt; ++t) {
            PB.set(i);
            M.swipe();
            double v=0; for (int k=0; k<nn; ++k) v += (k-nn2)*(k-nn2)*M[k].v*M[k].v; var[t]+=v;
        }
