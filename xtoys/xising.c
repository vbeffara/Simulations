...
void allocarray(char ***array, int xsize, int ysize, int datasize)
/*
 ...
 */
{
  ...
  allocarray((char***)&field, nrows, hwords, sizeof(long));
  allocarray((char***)&cluster, nrows, hwords, sizeof(long));
  allocarray((char***)&sadx, nrows, hwords, sizeof(long));
  allocarray((char***)&sady, nrows, hwords, sizeof(long));
  allocarray((char***)&unchecked, nrows, hwords, sizeof(long));
  allocarray((char***)&demon, DEMONBITS, ndemons, sizeof(long));
  allocarray((char***)&newdemon, DEMONBITS, ndemons, sizeof(long));
  if (NULL != work0)
    free(work0);
  if (NULL != work1)
    free(work1);
