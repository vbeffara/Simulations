...
void findpiv (long hcross)
{
  ...
  if (hcross==0) {
    for (x=0;x<n-1;x++)
      for (y=0;y<n;y++)
        if ((*img).at(coo(x,y))*(*img).at(coo(x+1,y))==2)
          pivh++;
    for (x=0;x<n;x++)
      for (y=0;y<n-1;y++)
        if ((*img).at(coo(x,y))*(*img).at(coo(x,y+1))==2)
          pivv++;
  } else {
    for (x=0;x<n-1;x++)
      for (y=0;y<n;y++)
        if ((*img).at(coo(x,y))*(*img).at(coo(x+1,y))==9) {
          pivh++;
        }
    for (x=0;x<n;x++)
      for (y=0;y<n-1;y++)
        if ((*img).at(coo(x,y))*(*img).at(coo(x,y+1))==9) {
          pivv++;
        }
  }
}
