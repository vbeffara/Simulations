...
vb::Color f_to_c (double f) {
    if (isnan(f)) return vb::RED; else
    if (f>0) return vb::Indexed(1); else
    return vb::Indexed(2);
}
...
class Wave : public vb::Sphere { public:
...
class Bargman : public vb::Sphere { public:
    Bargman (int n, int w) : vb::Sphere (w,[this](double x, double y, double z){ return f_to_c(v(x,y,z)); }), a(n+1), b(n+1), c(n+1), sqsq(n+1), n(n) {
        for (int i=0; i<=n; ++i) for (int j=0; j<=n-i; ++j) a[i].push_back(vb::prng.gaussian());
        ...
        for (int i=0; i<=n; ++i) for (int j=0; j<=n-i; ++j) eps = std::max (eps, abs(a[i][j]));
...
int main (int argc, char ** argv) {
    ...
    int s = vb::H['s']; if (s) vb::prng.seed(s);
    if (vb::H['t'] == "wave") { Wave F (vb::H['n'],vb::H['w']); F.show(); if (vb::H['p']) F.pause(); else F.output(); } else
    if (vb::H['t'] == "barg") { Bargman F (vb::H['n'],vb::H['w']); F.show(); if (vb::H['p']) F.pause(); else F.output(); } else
    { vb::H.L->error ("Type should be 'wave' or 'barg', exiting..."); exit(1); }
}
