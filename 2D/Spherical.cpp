...
double legendre_p (int l, int m, double x) { // l>=0, 0<=m<=l, -1<=x<=1
    ...
    for (int n=m+1; n<l; ++n) { 
        std::tie (p0,p1) = std::make_tuple (p1, ((2*n+1)*x*p1 - (n+m)*p0) / (n+1-m)); 
    }
    return p1;
}
...
int main (int argc, char ** argv) {
    ...
    int s = vb::H['s']; if (s != 0) { vb::prng.seed(s); }
    if (vb::H['t'] == "wave") { 
        Wave F (vb::H['n'],vb::H['w']); F.show(); if (vb::H['p']) { F.pause(); } else { F.output(); }
    } else if (vb::H['t'] == "barg") { 
        Bargman F (vb::H['n'],vb::H['w']); F.show(); if (vb::H['p']) { F.pause(); } else { F.output(); }
    } else { 
