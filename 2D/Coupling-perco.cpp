...
class Configuration : public Image { public:
		...
		bool dirty = true; while (dirty) {
			...
			for (int x=0; x<w(); x++) {
				for (int y=0; y<h(); y++) {
					coo z (x,y);
					for (int i=0; i<6; ++i) {
						coo zz = z+dz[i]; if (!expl.contains(zz)) continue;
						if ((expl[z]>0) && (expl[zz]>expl[z])) { expl[z] = expl[zz]; dirty=true; }
						if ((expl[z]<0) && (expl[zz]<expl[z])) { expl[z] = expl[zz]; dirty=true; }
					}
				}
			}
	...
	int nbarms (int r1, int r2, int sides) {
		...
		for (long i=-r1; i<r1; i++) {
			...
			k = expl[coo(N/2 + r1-1, N/2 + i   )]; if (table[N*N+k]==1) { table[N*N+k]=0; n++; }
		}

		return n;
	}

	bool test1 (int r1, int r2, int r3) {
		return (nbarms(r1,r3,15) == 4) && (nbarms(r2,r3,15) == 4) && (nbarms(r1,r2,15) == 4);
	}

	bool test2 (int r1, int r2, int r3) {
		return test1(r1,r2,r3) && (nbarms(r1,r3,6)==2);
	}
