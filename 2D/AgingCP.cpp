...
class Site { public:
	...
	operator Color() {
		if (a==0) return 0;
		else return Indexed(a);
	}
	int a;
};

vector<double> P { .005, .095, .2, .3, .4 };
...
class ACP : public Bitmap<Site> { public:
	...
	void run() {
		while (true) {
			...
			if (action == 0) { at(z)=0; continue; }
			if (action == 1) { int a = at(z); if ((a>0) && (a<5)) { at(z).a ++; } continue; }
			if (action > int(at(z))+1) continue;
			coo nz = z + dz[prng.uniform_int(4)];
			if (int(atp(nz)) == 0) putp(nz,1);
		}
	}
};
