...
class Domino { public:
	...
	operator Color () const {
		...
		return HSV (hh,1,.2+.8*(weight()-minw)/(maxw-minw));
	}

	coo z; int d; bool active;
};
...
class Tiling : public Bitmap<Domino> { public:
	...
	void run () { while (visible()) { flip(rand()); step(); } };
...
void two_by_two (double a) {
	...
	W[0][0][0]=a; W[0][0][1]=a; W[0][1][0]=a; W[1][0][1]=a; minw = min(a,1.0); maxw = max(a,1.0); if (maxw==minw) minw -= 1;
}

void three_by_two (double a) {
	W = vector<vector<vector<double>>> (6, vector<vector<double>> (2, vector<double> (2,1)));
	W[0][0][1]=a; W[3][1][1]=a; minw = min(a,1.0); maxw = max(a,1.0); if (maxw==minw) minw -= 1;
}
