...
public:
	...
	void fill (coo z) {
		if ( (z.x > 0) && (z.y < n-1) && at(z+coo{-1,1}) && !at(z-coo{1,0}) ) { put(z-coo{1,0},true); fill (z-coo{1,0}); }
		if ( (z.y > 0) && (z.x < n-1) && at(z+coo{1,-1}) && !at(z-coo{0,1}) ) { put(z-coo{0,1},true); fill (z-coo{0,1}); }
	}
	...
	void run () {
		while (true) {
			...
			if (!at(z)) {
				double curtime = pt.t;
				if (ok(z)) {
					put(z,true); cursum = max (cursum, z.x+z.y); if (f) fill (z);
