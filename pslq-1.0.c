...
#define OUT(a) { mpf_out_str (stdout, 10, 0, a); printf ("\n"); }
#define mpz_addmul_si(a,b,c) do { \
           if (c >= 0) \
              mpz_addmul_ui(a,b,(unsigned long)c); \
           else \
              mpz_submul_ui(a,b,(unsigned long)-c); } while (0)
#define mpz_submul_si(a,b,c) do { \
           if (c >= 0) \
              mpz_submul_ui(a,b,(unsigned long)c); \
           else \
              mpz_addmul_ui(a,b,(unsigned long)-c); } while (0)
#define mpf_mul_si(a,b,c) do { \
           if (c >= 0) \
...
void
scalprod (mpf_t s, mpf_t *x, mpz_t *y, unsigned long n)
{
  unsigned long i;
  mpf_t t;
...
void
mul_matrix_vector (mpf_t *y, mpz_t **B, mpf_t *x, unsigned long n)
{
  size_t s, maxsize;
  unsigned long i, j;
  mpf_t t, u;
 ...
 */
void
pslq (mpz_t *y, mpf_t *x0, unsigned long n, double gam, int verbose,
      char *input_A, char *output_A, char *input_B, char *output_B, int report)
{
  mpz_t **A, **B, tt, *Bp;
  mpf_t **H, *Hp, *s, *x;
  ...
  */
      ...
      for (j = i - 1; j >= 0; j--)
	{
	  ...
	  for (k = 0; k < n; k++)
		...
		mpz_submul (A[i][k], tt, A[j][k]);
              ...
              if ((size = mpz_sizeinbase (B[j][k], 2)) > size_B)
                size_B = size;
            }
	}
    }
  ...
  do
    {
      ...
      /* step 2 */
      mpz_swap (y[m], y[m+1]);
      Hp = H[m]; H[m] = H[m+1]; H[m+1] = Hp;
      if (need_A)
	{
	  Bp = A[m]; A[m] = A[m+1]; A[m+1] = Bp;
	}
      Bp = B[m]; B[m] = B[m+1]; B[m+1] = Bp;
      ...
      if (verbose && (iter % report == 0))
	{
	  ...
	  mpf_ui_div (u, 1, u);
	  printf ("iter=%u M=", iter);
	  mpf_out_str (stdout, 10, 3, u);
	  printf (" ymin=%u", mpz_sizeinbase (y[j], 2));
	  printf (" ymax=%u", k);
	  printf (" time=%ds", (cputime () - st) / 1000);
	  printf (" (%dms/it)", (cputime () - st) / iter);
	  printf ("\n");
          fflush (stdout);
	}
      ...
      if (k + prec / 10 + 20 < prec)
	{
	  prec = k + 10;
          if (verbose >= 2)
            printf ("Reducing precision to %u\n", prec);
  ...
  if (output_A != NULL)
    {
      if (verbose >= 2)
        printf ("Storing matrix A in file %s\n", output_A);
  ...
  if (output_B != NULL)
    {
      if (verbose >= 2)
        printf ("Storing matrix B in file %s\n", output_B);
...
main (int argc, char *argv[])
{
  ...
  char *input_A = NULL, *output_A = NULL;
  ...
  if (verbose >= 2)
    printf ("Using precision=%u, gamma=%1.20e\n", prec, gamma);
  ...
  pslq (rel, x, n, gamma, verbose, input_A, output_A, input_B, output_B,
	report);
