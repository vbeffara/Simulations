...
I am using the very nice `waf <http://code.google.com/p/waf/>`_ 
...
copy somewhere in your ``$PATH`` for convenience (somewhere where you 
have write privileges while running it), or use directly.
...
<http://thy.phy.bnl.gov/www/xtoys/xtoys.html>`_. I include them because 
they are somehow related to the others, and because I had to rewrite the 
Makefile to compile them on Mac OS X, now they should compile everywhere 
(as long as the X11 include files are present, of course).  If there is 
any (legal or other) problem about their presence here, I will remove 
them at once.
