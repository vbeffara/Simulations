...
struct Expression;
struct Number : public real_t {
    using real_t::real_t;
};
struct Symbol : public string {
    using string::string;
};
struct Plus : public vector<Expression> {
    using vector<Expression>::vector;
};

using Expression_ = variant<Number, Symbol, Plus>;
struct Expression : public Expression_ {
...
struct flattener {
    ...
    Expression                       operator()(const Plus & p) const {
        Plus sum;
        for (const auto & e : p)
            for (const auto & ee : convert<Plus>(visit(*this, e))) sum.push_back(ee);
        return sum;
    }
};
...
struct normalizer {
    ...
    Expression                       operator()(const Plus & p) const {
        ...
        Number acc{0};
        for (const auto & e : p)
            if (auto n = get_if<Number>(&e))
                acc += *n;
            else
                out.push_back(visit(*this, e));
