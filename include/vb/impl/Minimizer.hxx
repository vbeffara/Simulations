...
namespace vb {
    template <typename T> Minimizer<T>::Minimizer(size_t n_) : n(n_), x(n), gx(n), old_x(n), old_gx(n), er(1.0) {}

    template <typename T> Minimizer<T>::Minimizer(size_t n_, std::function<T(const Vector<T> &, Vector<T> *)> fg_) : Minimizer(n_) {
        fg = fg_;
    }

    template <typename T>
    Minimizer<T>::Minimizer(size_t n_, std::function<T(const Vector<T> &)> f_, std::function<Vector<T>(const Vector<T> &)> g_)
        : Minimizer(n_) {
        f = f_;
        g = g_;
    }
