...
namespace vb {
  ...
  template <typename T> auto coo_range(coo_2d<T> r1, coo_2d<T> r2) -> Stream2<coo_2d<T>> {
    for (T y = r1.y; y < r2.y; ++y)
      for (T x = r1.x; x < r2.x; ++x) co_yield coo_2d<T>{x, y};
  }

  template <typename T> auto coo_range(coo_2d<T> r) -> Stream2<coo_2d<T>> { return coo_range({0, 0}, r); }
} // namespace vb
