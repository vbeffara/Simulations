...
public:
    int dist_to_box(coo z) { return max(int64_t(0), min({z.x - (ul.x + w - 1), z.y - (ul.y + w - 1), ul.x - z.x, ul.y - z.y})); }
    int dist_to_out(coo z) { return max(int64_t(0), min({z.x - ul.x + 1, z.y - ul.y + 1, (ul.x + w) - z.x, (ul.y + w) - z.y})); }
