...
namespace vb {
	...
	void Cluster::ensure_sub () {
		if (!sub.empty()) return;
		int64_t ww = w/3;
		for (int64_t y=0; y<3; ++y) for (int64_t x=0; x<3; ++x) sub.emplace_back (ul + coo(x*ww,y*ww), ww);
	...
	bool Cluster::at (coo z) const {
		...
		int64_t x = z.x - ul.x, y = z.y - ul.y, xy = x + w*y; return tile[xy];
	}

	void Cluster::put (coo z, bool b) {
		int64_t plain = b ? w*w : 0; if (np == plain) return;
		if (w == bs) {
	...
	void Cluster::validate () {
		...
		if (!sub.empty()) {
			int64_t s=0; for (auto & c : sub) { c.validate(); s += c.np; } assert (s==np);
		}
	}
	...
	void Cluster::paint (Image & I, coo ul, coo br) {
		...
		if (np == 0) { for (int64_t x=ul.x; x<br.x; ++x) for (int64_t y=ul.y; y<br.y; ++y) I.put(coo(x,y),BLACK); }
		else if (np == w*w) {
			for (int64_t x=ul.x; x<br.x; ++x) for (int64_t y=ul.y; y<br.y; ++y)
			...
			for (int64_t x=0; x<3; ++x) for (int64_t y=0; y<3; ++y)
				sub[x+3*y].paint (I,ul+coo(x*ww/3,y*hh/3),ul+coo((x+1)*ww/3,(y+1)*hh/3));
		} else {
			int64_t ww = br.x-ul.x, s = w/ww;
			for (int64_t x=0; x<ww; ++x) for (int64_t y=0; y<ww; ++y)
				I.put (ul+coo(x,y), tile[x*s+w*y*s] ? WHITE : BLACK);
		}
	}

	int64_t Cluster::sub_index (coo z) const { int64_t ww = w/3, i = (z.x-ul.x)/ww, j = (z.y-ul.y)/ww; return i+3*j; }
