...
namespace vb {
	template <typename T> Constellation0<T>::Constellation0 (const Hypermap & M, Hub & H) {
		...
		while (true) {
			...
			for (auto c : M.phi.cycles()) { int i = S.E[c[0]+3*N].src, d = S.V[i].adj.size(); if (d>dinf) { inf=i; dinf=d; } }
			S.linear (1,-S.V[inf].z); S.inversion(); S.linear (-1/S.V[inf].r,0); S.mobiusto0 (S.V[S.E[0].src].z);
			b.clear(); for (auto c : M.sigma.cycles())	{                              	b.push_back( { to_cpx<T>(S.V[S.E[c[0]].src].z),    	int(c.size()) } ); }
			w.clear(); for (auto c : M.alpha.cycles())	{                              	w.push_back( { to_cpx<T>(S.V[S.E[c[0]+N].src].z),  	int(c.size()) } ); }
			f.clear(); for (auto c : M.phi.cycles())  	{ if (S.E[c[0]+3*N].src != inf)	f.push_back( { to_cpx<T>(S.V[S.E[c[0]+3*N].src].z),	int(c.size()) } ); }
	...
	template <typename T> void Constellation0<T>::make_p_1 () {
		...
		Polynomial<cplx> P; for (auto zd : b) for (int j=0; j<zd.d; ++j) P.add_root(zd.z);
		Polynomial<cplx> Q; for (auto zd : f) for (int j=0; j<zd.d; ++j) Q.add_root(zd.z);
		int i=0; while (norm(P[i])<eps) ++i;
		int j=0; while (norm(Q[j])<eps) ++j;
		bool is_P; cplx l(1);
	...
	template <typename T> auto Constellation0<T>::vcost() const -> Vector<cplx> {
		...
		for (auto zd : w) for (int j=0; j<zd.d; ++j) out[k++] = logder(zd.z,j);
	...
	template <typename T> auto Constellation0<T>::jacvcost () const -> Matrix<cplx> { // m_ij = \partial_j(f_i)
		...
		int i=0,j=0; for (int ii=0; ii<w.size(); ++ii) for (int id=0; id<w[ii].d; ++id) { j=0;
			for (int jj=0; jj<b.size(); ++jj)	if (id==0)     	out(i,j++) = T(- T(b[jj].d)) / (w[ii].z-b[jj].z);
			                                      	else           	out(i,j++) = T(id*b[jj].d) / pow(w[ii].z-b[jj].z,cplx(id+1));
			for (int jj=0; jj<w.size(); ++jj)	if (jj!=ii)    	out(i,j++) = T(0);
			                                      	else if (id==0)	out(i,j++) = logder(w[ii].z,1);
			                                      	else           	out(i,j++) = T(- T(id)) * logder(w[ii].z,id+1);
			for (int jj=0; jj<f.size(); ++jj)	if (id==0)     	out(i,j++) = T(f[jj].d) / (w[ii].z-f[jj].z);
			                                      	else           	out(i,j++) = T(- T(id*f[jj].d)) / pow(w[ii].z-f[jj].z,cplx(id+1));
			++i;
		}
		j=0; for (int jj=0; jj<b.size(); ++jj) out(i,j++) = T(b[jj].d); while (j<dim) out(i,j++) = T(0); ++i;
		return out;
	}
	...
	template <typename T> std::ostream & operator<< (std::ostream & os, const Constellation0<T> & C) {
		...
		Polynomial <typename cpx_t<T>::type> P; for (auto zd : C.b) for (int j=0; j<zd.d; ++j) P.add_root(zd.z); os << "P[z_] := " << P << std::endl;
		Polynomial <typename cpx_t<T>::type> Q; for (auto zd : C.f) for (int j=0; j<zd.d; ++j) Q.add_root(zd.z); os << "Q[z_] := " << Q << std::endl;
		return os;
	}
	...
	template <> std::ostream & operator<< (std::ostream & os, const Constellation0<real_t> & C) {
		...
		Polynomial<complex_t> P; for (auto zd : C.b) for (int j=0; j<zd.d; ++j) P.add_root(zd.z);
		for (auto & x : P) {
			auto xx = cln::complex (round1(realpart(x)), round1(imagpart(x)));
			if (abs(x - xx) < pow(T(.1),nd)) x = xx;
		}
		os << "P[z_] := " << P << std::endl;
		Polynomial<complex_t> Q; for (auto zd : C.f) for (int j=0; j<zd.d; ++j) Q.add_root(zd.z);
		for (auto & x : Q) {
			auto xx = cln::complex (round1(realpart(x)), round1(imagpart(x)));
			if (abs(x - xx) < pow(T(.1),nd)) x = xx;
		}
	...
	template <typename T> boost::optional<Hypermap> Constellation0<T>::explore () const {
		...
		int maxdeg=0; for (auto z : Z) maxdeg = std::max (maxdeg, z.d);
