...
namespace vb {
	...
	template <typename T> auto Elliptic<T>::theta1_z (const cplx & z) const -> cplx {
		return sum ([&](int n) { return T(2 * pow(T(-1),n)) * q14 * pow(q, n*(n+1)) * T(2*n+1) * cos(T(2*n+1)*z); });
	}

	template <typename T> auto Elliptic<T>::theta1_q (const cplx & z) const -> cplx {
		return sum ([&](int n) { return T(2 * pow(T(-1),n)) * (n+T(1)/2) * (n+T(1)/2) * q14 * pow(q, n*(n+1)-1) * sin(T(2*n+1)*z); });
	}

	template <typename T> auto Elliptic<T>::theta1_zq (const cplx & z) const -> cplx {
		return sum ([&](int n) { return T(2 * pow(T(-1),n)) * (n+T(1)/2) * (n+T(1)/2) * q14 * pow(q, n*(n+1)-1) * T(2*n+1) * cos(T(2*n+1)*z); });
	}

	template <typename T> auto Elliptic<T>::theta1_zz (const cplx & z) const -> cplx {
		return sum ([&](int n) { return T(-2) * q14 * pow(q, n*(n+1)) * pow(T(2*n+1),2) * sin(T(2*n+1)*z); });
	}

	template <typename T> auto Elliptic<T>::theta1_zzq (const cplx & z) const -> cplx {
		return sum ([&](int n) { return - T(2 * pow(T(-1),n)) * (n+T(1)/2) * (n+T(1)/2) * q14 * pow(q, n*(n+1)-1) * pow(T(2*n+1),2) * sin(T(2*n+1)*z); });
	}

	template <typename T> auto Elliptic<T>::theta1_zzz (const cplx & z) const -> cplx {
		return sum ([&](int n) { return - T(2 * pow(T(-1),n)) * q14 * pow(q, n*(n+1)) * pow(T(2*n+1),3) * cos(T(2*n+1)*z); });
	}

	template <typename T> auto Elliptic<T>::theta1_zzzq (const cplx & z) const -> cplx {
		return sum ([&](int n) { return - T(2 * pow(T(-1),n)) * (n+T(1)/2) * (n+T(1)/2) * q14 * pow(q, n*(n+1)-1) * pow(T(2*n+1),3) * cos(T(2*n+1)*z); });
	}
	...
	template <typename T> auto Elliptic<T>::theta2_q (const cplx & z) const -> cplx {
		return sum ([&](int n) { return T(2) * (n+T(1)/2) * (n+T(1)/2) * q14 * pow(q, n*(n+1)-1) * cos(T(2*n+1)*z); });
	}
	...
	template <typename T> auto Elliptic<T>::eta1 () const -> cplx {
		return - pi_<T>()*pi_<T>()/T(6) * theta1_zzz(T(0)) / theta1_z(T(0));
	}
	...
	template <typename T> auto Elliptic<T>::g3 () const -> cplx {
		cplx a=theta2(cplx(0)), b=theta3(cplx(0));
		return pow(pi_<T>(),6)/(pow(T(3)/2,3))*(pow(a,12)-(T(3)/2)*(pow(a,8)*pow(b,4)+pow(a,4)*pow(b,8))+pow(b,12));
	}
