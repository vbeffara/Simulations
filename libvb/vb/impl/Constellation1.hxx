...
namespace vb {
	template <typename T> Constellation1<T>::Constellation1 (const Hypermap & M, Hub & H) {
		...
		do {
			...
			f.clear(); for (auto c : M.phi.cycles())  	f.push_back( { to_cpx<T>(S.V[S.E[c[0]+3*N].src].z),	int(c.size()) } );
	...
	template <typename T> auto Constellation1<T>::jacvcost () const -> Matrix<cplx> { // m_ij = \partial_j(f_i)
		...
		int i=0,j=0; for (int ii=0; ii<w.size(); ++ii) for (int id=0; id<w[ii].d; ++id) { j=0; // f_i is logder(w[ii],id)
