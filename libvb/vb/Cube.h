...
namespace vb {
	...
	class Cube : public Bitmap <Adder> { public:
		...
		void	putp	(const coo3 &c, unsigned char t)	{ put(wrap(c),t); }
		bool	done	(const coo3 &c)                 	{ return (c.z==sz); }

		unsigned char &	at 	(const coo3 &c) { return data[index(c)]; }
		unsigned char &	atp	(const coo3 &c) { return at(wrap(c)); }

		using iterator = Cube_iterator; iterator begin (); iterator end ();

		using Bitmap <Adder> :: at;
		void put (const coo3 &c, unsigned char t) {
