...
namespace vb {
	Hub::Hub (std::string t, int argc, char ** argv, std::string c) : title(t), help("Syntax : " + c), getopt_arg("h") {
			...
			if (params.size()) {
				bool first = true;
				o << " (";
				for (auto i : params) { o << (first ? "" : ",") << i.first << "=" << i.second; first = false; }
				o << ")";
			}
			title += o.str();
		}
	...
	void Hub::newparam (const std::string &s) {
		if (s.length() == 1)	{ getopt_arg += s.substr(0,1);      	params[s[0]] = "0";        	has_arg[s[0]] = false; }
		else                	{ getopt_arg += s.substr(0,1) + ':';	params[s[0]] = s.substr(2);	has_arg[s[0]] = true; }
	}
	...
	void Hub::parse (int argc, char **argv) {
		...
		while ((ch = getopt(argc,argv,getopt_arg.c_str())) != -1) {
			...
			else if (has_arg[ch])	{ params[ch] = optarg; }
			else                 	{ params[ch] = "1"; }
		}
	}
}
