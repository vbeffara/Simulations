...
namespace vb {
    ...
    void Coloring::do_aa() {
        ...
        tbb::parallel_for(0, to_signed(pw * ph), [=, this](int ii) {
    ...
    void Coloring::line(ucoo s, coo d, size_t l) {
        tbb::parallel_for(size_t(0), l, [=, this](size_t ii) {
            ucoo const c = s + d * ii;
    ...
    void Coloring::tessel_go(ucoo ul, ucoo lr) {
        ...
        line(ul_, dd_, size);
        tbb::task_group G;
        G.run([=, this]() { tessel_go(ul, lr_); });
        G.run([=, this]() { tessel_go(ul_, lr); });
        G.wait();
    }

    void Coloring::tessel(ucoo ul, ucoo lr) {
        tbb::task_group G;
        G.run([=, this]() { line(ul, {1, 0}, lr.x - ul.x); });
        G.run([=, this]() { line({lr.x, ul.y}, {0, 1}, lr.y - ul.y); });
        G.run([=, this]() { line(lr, {-1, 0}, lr.x - ul.x); });
        G.run([=, this]() { line({ul.x, lr.y}, {0, -1}, lr.y - ul.y); });
        G.wait();
        tessel_go(ul, lr);
    }
