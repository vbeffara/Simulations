...
namespace vb {
    ...
    Sphere::Sphere (int w, const std::function <Color(double,double,double)> &f) : Sphere (w, [f](cpx z){
        double x1=real(z), x2=imag(z), n = x1*x1 + x2*x2; if (n>1) return NOCOLOR;
        double x3=sqrt(1-n); return f(x1,x2,x3);
    }) {}

    Sphere::Sphere (int w, const std::function <Color(double,double)> &f) : Sphere (w, [f](cpx z){
        double x1=real(z), x2=imag(z), n = x1*x1 + x2*x2; if (n>1) return NOCOLOR;
