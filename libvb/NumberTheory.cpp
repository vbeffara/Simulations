...
namespace vb {
	...
	boost::optional<cl_UP_R> guess (cl_F x, int nd) {
		...
		for (int d=1; d<=nd/10; ++d) {
			...
			auto PP = deriv (P); cl_F xx=x, ox=x+1; while (abs(1-xx/ox) > expt(cl_float(10),5-nd)) { ox = xx; xx -= P(xx)/PP(xx); }
	...
	boost::optional<cl_UP_N> guess_c (cl_N x, int nd) {
		...
		for (int d=1; d<=nd/10; ++d) {
			...
			auto PP = deriv (P); cl_N xx=x, ox=x+1; while (abs(1-xx/ox) > expt(cl_float(10),5-nd)) { ox = xx; xx -= P(xx)/PP(xx); }
			if (abs(1-xx/x) < expt(cl_float(10),10-nd)) return P;
		}
