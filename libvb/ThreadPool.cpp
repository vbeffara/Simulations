...
namespace vb {
    ...
    static void loop_go(BLST &S, std::shared_ptr<Will> parent, int a, int b, const std::function<void(int)> &f, int l) {
        if (b - a <= l) {
            for (int i = a; i < b; ++i) f(i);
            return;
        }
        int c = (a + b) / 2;
        S.push([=, &S, &f] { loop_go(S, parent, a, c, f, l); });
        S.push([=, &S, &f] { loop_go(S, parent, c, b, f, l); });
    };

    void loop_par(int a, int b, const std::function<void(int)> &f, int l) {
        run_par([=, &f](auto &S, auto p) { loop_go(S, p, a, b, f, l); });
    }
