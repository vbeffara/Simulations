...
namespace vb {
    Map::Map(const std::string &s, int nn) : Picture(s, {600, 600}), n(nn), zero(-1), one(-1), infinity(-1) {
        for (int i = 0; i < n; ++i) {
    ...
    double Map::balance() {
        ...
        for (int i = 0; i < n; ++i) { v[i]->z = cpx(M.x[2 * i], M.x[2 * i + 1]); }
    ...
    int Map::nb_faces() {
        double tmp = 0.0;
        for (int i = 0; i < n; ++i)
            for (int j : v[i]->adj) tmp += double(1.0) / double(face(Edge(i, j)).size());
        return floor(tmp + .1);
    }

    int Map::euler() { return nb_sommets() - nb_aretes() + nb_faces(); }
